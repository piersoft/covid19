...
<div id="infodivtitolo" style="leaflet-popup-content-wrapper" >
	<p>
		<b>
	<?php
 if (htmlspecialchars($_GET["incidenza"])=="1")	echo 'Covid19 - Progressivo decessi ogni 10000"</br><font color="red">USARE LO SLIDER IN BASSO A DESTRA</font>' ?>
 <?php
...
$url="datageo.txt";
		 ...
		 var f=1; //fattore di divisione per i colori
	...
	function style10000(feature,min,max) {
		return {
			...
			dashArray: '3',
			fillOpacity: 0.7,
			fillColor: getColor1000(feature.properties.decessipopolazione)
		};
	}
	...
	legend.onAdd = function (map) {
	    ...
	    var div = L.DomUtil.create('div', 'info legend1'),
			grades = [0,0.1,0.5,1,2,4,10]	        ,
	        labels = [];


					var f1=1;					//console.log(grades);
	    // loop through our density intervals and generate a label with a colored square for each interval
	    for (var i = 0; i < grades.length; i++) {
					console.log((grades[i+1]*f1).toFixed(2)+ ' i: '+i);
	            ...
	            '<i style="background:' +
							...
							getColor1000(grades[i]/f1/1)							 + '"></i> ' +
	            (grades[i]*f1).toFixed(2) + (typeof grades[i+1] !== 'undefined' ? '&ndash;' + (grades[i+1]*f1).toFixed(2) + '<br>' : '+');
	    }

//(grades[i]*f1).toFixed(2) + (typeof grades[i+1] !== 'undefined' ? '&ndash;' + (grades[i+1]*f1).toFixed(2) + '<br>' : ' e pi&ugrave;');
	    return div;
	};
	...
	dataLayer = L.geoJson(statesData, {
		...
		console.log(feature.properties.totalicasiattiviregione);
			...
			//	echo "popupString += 'Casi totali attivi: <b>' + feature.properties.totalicasiattiviregione + '</b><br />';";
				...
				echo "popupString += 'Tasso mortalità ogni 10000 abitanti: <b>' + feature.properties.decessipopolazione + '</b><br />';";
?>	if (document.body.clientWidth <= 767) {
	<?php echo "popupString += '<a href=\"filternewreg.php?regione='+ reg+'&data=deceduti\"\">Vai alla serie storica</a></br>';"; ?>
}else{
	<?php echo "popupString += '<div><iframe src=\"filternewreg.php?regione='+ reg +'&data=deceduti\"\" width=\"280\" scrolling=\"no\" height=\"320\" frameBorder=\"0\">prova</iframe></div>';";?>
...
var casistica = [];
		...
		//		if (layer.feature.properties.name = element.properties.prov_name){
			//			element.properties.casi=newElement[timestamp];
				element.properties.decessipopolazione=(newElement[timestamp]/newElement.popolazione*10000).toFixed(2);
