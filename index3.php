...
<div id="infodivtitolo" style="leaflet-popup-content-wrapper" >
	<p>
		<b>
			<?php
			if (htmlspecialchars($_GET["incidenza"])=="1")	echo 'Covid19 - Progressivo contagio "attivi al"</br><font color="red">USARE LO SLIDER IN BASSO A DESTRA</font>' ?>
			<?php
...
<div>
	<div id="infodivfocuscom" style="leaflet-popup-content-wrapper">
		...
		<div>
			...
			<script type="text/javascript" src="<?php if (htmlspecialchars($_GET["incidenza"])=="2" || htmlspecialchars($_GET["incidenza"])=="3")	echo ('regioni2.js'); else echo ('regioni2.js'); ?>"></script>
			...
			function stylereg(feature) {
				...
				return {
					...
					dashArray: '3',
					fillOpacity: 0.7,
					fillColor: getColor(feature.properties.totalicasiattiviregione)
				};
			}
			...
			legend.onAdd = function (map) {
				...
				if (htmlspecialchars($_GET["incidenza"])=="1" ){
					...
					echo 'grades = [0,50, 200, 500, 1000, 2000, 5000, 10000,15000,100000]';
			...
			function join(data){
				...
				var primo="<?php if (htmlspecialchars($_GET["incidenza"])=="1" || htmlspecialchars($_GET["incidenza"])=="3") {
					echo ('element.properties.Regione');
				}else{
					echo ('element.properties.prov_acr');
				}?>";

				var secondo="<?php if (htmlspecialchars($_GET["incidenza"])=="1" || htmlspecialchars($_GET["incidenza"])=="3") {
				...
				statesData.features.forEach(function(element){
					...
					data.find(function(newElement) {
						...
						if(eval(primo)==eval(secondo))
						{
							...
							if (newElement.nuovi_positivi != undefined) {
								element.properties.nuovi_attualmente_positivi=newElement.nuovi_positivi;
								element.properties.totale_positivi=newElement.totale_positivi;
							}
			...
			function addDataToMap(value) {
				dataLayer = L.geoJson(statesData, {
					onEachFeature: function(feature, layer) {
						...
						//	console.log('sono qui con Regione: '+reg);

						var positivi='totale_attualmente_positivi';
						//console.log('tot pos'+feature.properties.totale_positivi);
						//if (feature.properties.totale_positivi != undefined) positivi='totale_positivi';
						...
						<?php if (htmlspecialchars($_GET["incidenza"])=="1"){
							...
							?>	if (document.body.clientWidth <= 767) {
								<?php echo "popupString += '<a href=\"filternewreg.php?regione='+ reg+'&data=totale_positivi\"\">Vai alla serie storica</a></br>';"; ?>
							}else{
								<?php echo "popupString += '<div><iframe src=\"filternewreg.php?regione='+ reg +'&data=totale_positivi\"\" width=\"280\" scrolling=\"no\" height=\"320\" frameBorder=\"0\">prova</iframe></div>';";?>
							...
							?>	if (document.body.clientWidth <= 767) {
								...
								<?php echo "popupString += '<div><iframe src=\"filternew.php?regione='+ reg +'&data=nuovi_attualmente_positivi\" width=\"280\" scrolling=\"no\" height=\"320\" frameBorder=\"0\">prova</iframe></div>';";?>
	...
	*/
	...
	var bankias = L.geoCsv(null, {
		onEachFeature: function (feature, dataLayer1) {
			...
			popupString += '<b>' + feature.properties.totalicasiattiviregione + '</b>';
			popupString += '</div>';
		...
		pointToLayer: function (feature, latlng) {
			...
			var casistica=feature.properties.totalicasiattiviregione;

			<?php if (htmlspecialchars($_GET["incidenza"])=="1"){

				echo 'casistica=feature.properties.totalicasiattiviregione;';
			}?>
			...
			return L.marker(latlng,
				...
				function createSliderUI(timestamps,min,max) {
					...
					buttonControl = L.control({ position: 'bottomright'} );
			...
			function updatePropSymbols(timestamp) {
				...
				cities.eachLayer(function(layer) {
					...
					//		console.log('Regione '+layer.feature.properties.regione);
					//		console.log('Totale casi attivi '+layer.feature.properties.totalicasiattiviregione);
					...
					//	var incidenza= props.totalicasiattiviregione;
				...
				statesData.features.forEach(function(element){
					casistica.find(function(newElement) {
						...
						if(newElement.regione==element.properties.Regione)
						{
							//		if (layer.feature.properties.name = element.properties.prov_name){
							//			element.properties.casi=newElement[timestamp];
							//			element.properties.popolazione=newElement.popolazione;
							element.properties.totalicasiattiviregione=newElement[timestamp];
							//		element.properties.totalicasiattiviregione=Math.round((newElement[timestamp]/element.properties.totalicasiattiviregione)*10000 * multiplier) / multiplier ;
							//		console.log('Ci sono '+JSON.stringify(newElement[timestamp]))
							//	console.log(' stai: '+newElement.regione);
						}
					});
					//	}
			...
			function processData(data) {
				...
				for (var feature in data.features) {
					...
					for (var attribute in properties) {
						...
						attribute != 'regione' &&
						attribute != 'lat' &&
						attribute != 'denominazione_regione' &&
						attribute != 'long' ) {
							...
							if (properties[attribute] < min10000) {
								//		min10000 = properties.totalicasiattiviregione;
							}

							if (properties[attribute] > max10000) {
								//		max10000 = properties.totalicasiattiviregione;
							}
			...
			function play(timestamps) {         //  create a loop function
				map.removeControl(sliderControl);

				for (let i=0; i<timestamps.length; i++) {
					setTimeout( function timer(){
						updatePropSymbols(timestamps[i]);
