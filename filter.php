...
<html lang="en">
  ...
  <!--  <script src="assets/scripts4.js"></script>-->
    ...
    $.getJSON(url, function(response) {
      ...
      var output = outputtmp.map( s => ({x:s.data, y:s.totale_attualmente_positivi}) );

      var output1 = response.map( s => ({x:s.data, y:s.totale_attualmente_positivi}) );

      var output2 = response.map( s => ({x:s.data, y:s.totale_attualmente_positivi}) );

      var output3 = response.map( s => ({x:s.data, y:s.totale_attualmente_positivi}) );
